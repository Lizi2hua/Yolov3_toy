...
IOU_THRESH=0.5
class Detector():
    #todo:这个里面 tosquare和resize是重复的操作，需要和外面调用的代码重构；重写代码
    def __init__(self):
        self.net = YOLOv3(20, use_mode='shufflenet').cuda()
        # self.net = YOLOv3(20, use_mode='darknet53').cuda()
        ...
        #     print('未找到预训练模型文件')
        #从外部加载模型
        target_pt=r'C:\Users\liewei\Desktop\yolo_pt_version\witout_alpha_pt\shufflenet_few_version.pt'
        self.net.load_state_dict(torch.load(target_pt))
        self.net.eval()
    ...
    def __call__(self,img_data ):
        '''
        :param :是正方形且416*416的图片
        :return:
        '''
        start_time=time.time()
        #传入PIL对象，得到3个结果
        square_img=self.ToSquare(img_data)
        ...
        if idx_52.shape[0]!=0:
            boxes_52=self.__parse(idx_52,vec_52,416/52,ANCHOR_BOXES[52])
